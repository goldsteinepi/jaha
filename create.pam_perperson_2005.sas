...
* A valid person is a person with 1+ valid days.                                           *;
...
libname demo xport "W:\goldstein\2005\demo_d.xpt";
...
data myfolder.pam_perperson;
  merge myfolder.pam_all(in=in_pam keep=seqn valid_person valdays) demo.demo_d(in=in_demog) myfolder.valid_days;* changed by kwd;
  by seqn;
  if in_pam;
...
data myfolder.pam_perperson_2005;
  set myfolder.pam_perperson;
run;

proc contents data=myfolder.pam_perperson_2005;
run;
...
data maurice;
set myfolder.pam_perperson_2005;
where RIDAGEYR>=40;
run;
