...
*----------------------------------------------------------------------------*;
libname myfolder "Z:\OneDrive - McGill University\goldstein";
%include "Z:\OneDrive - McGill University\goldstein\PAM_formats.sas";

libname xptfile xport "Z:\OneDrive - McGill University\goldstein\paxraw_d.xpt" access=readonly;

proc copy inlib=xptfile outlib=myfolder;
run;
...
*----------------------------------------------------------------------------*;
data paxraw (compress=yes reuse=yes);
  set myfolder.paxraw_d;
  day=ceil(paxn/1440);
...
data invalid valid (compress=yes reuse=yes);
  set paxraw;
  by seqn;
...
data paxraw_invalid (compress=yes reuse=yes);
...
proc sort data=paxraw out=myfolder.paxrawsorted;
   by seqn day paxn ;
run;
 ...
 *PAXSTAT=2;
data pam_perminute;
  update  myfolder.paxrawsorted paxraw_new;
  by seqn day paxn ;
...
data myfolder.pam_perminute_2005;
  set pam_perminute;
run;

proc contents data=myfolder.pam_perminute_2005;
run;
