...
*4. Create a SAS dataset named demo_d in that folder from the Demographic data from                      *;
*   http://www.cdc.gov/nchs/about/major/nhanes/nhanes2003-2004/demo03_04.htm.                            *;
*--------------------------------------------------------------------------------------------------------*;
libname myfolder "W:\goldstein\2005";
%include "W:\goldstein\original-code\PAM_formats.sas";

libname demo xport "W:\goldstein\2005\demo_d.xpt";* Added by kwd;
proc copy inlib=demo outlib=myfolder;
run;
...
*---------------------------------------------------------------------*;
data myfolder.monitors;
*  merge myfolder.pam_perminute(in=in_pam) myfolder.demo_d(in=in_demog keep=seqn ridageyr);
  merge myfolder.pam_perminute_2005(in=in_pam) demo.demo_d(in=in_demog keep=seqn ridageyr);* Added by kwd;
  by seqn;
  if in_pam;
...
*--------------------------------------------------------------------------------------------------------*;
%macro nw(nwperiod=);
data myfolder.nw_all;
  set myfolder.monitors;
  by seqn day paxn;
...
*---------------------------------------------------------------------*;
proc summary data=myfolder.nw_all ;
  by seqn day;
  var dur_nw ;
  output out=myfolder.sum_nw
       sum=tot_dur_nw
run;
...
*-------------------------------------------------------------------------*;
proc summary data=myfolder.monitors;
  by seqn day ridageyr paxday;
  var paxinten;
  output out=myfolder.sum_all
         n=tot_min;
run;
...
data myfolder.nw_minutes(keep=seqn day paxn);
  set myfolder.nw_all;
  by seqn day nw_num;
...
data myfolder.wear_minute(keep=seqn day paxn paxinten);
  merge myfolder.monitors(in=in_all) myfolder.nw_minutes(in=in_nw);
  by seqn day paxn;
  if in_all and not in_nw;
run;

/*summarize the wear minutes */
proc summary data=myfolder.wear_minute;
  by seqn day;
  var paxinten;
  output out=myfolder.sum_wear
         sum=tot_cnt_wr
         n=tot_min_wr;
run;
...
*---------------------------------------------------------------------*;
data myfolder.nw&nwperiod;
  merge myfolder.sum_all(in=in_all) myfolder.sum_nw(in=in_nw) myfolder.sum_wear;
  by seqn day;
  if in_all;
...
data myfolder.monitors;
  ...
  else if ridageyr>=18 then modthresh=760;
  ...
  else if ridageyr>=18 then vigthresh=2020;
...
proc sort data=myfolder.monitors; by seqn day paxn; run;
proc means data=myfolder.monitors;
var _mv _v _m;
run;
...
data myfolder.out&bout_flg&boutperiod;
  set myfolder.monitors;
  by seqn day paxn;
...
proc sort data=myfolder.out&bout_flg&boutperiod;
  by seqn day mv_num;
run;
...
*each day.                                      *;
*-----------------------------------------------*;
proc summary data=myfolder.out&bout_flg&boutperiod;
  by seqn day;
  var dur_mv;
  output out=myfolder.sum_mv
         sum=tot_dur_mv;
run;
...
*the analysis.                                  *;
*-----------------------------------------------*;
data myfolder.out&bout_flg&boutperiod._sum;
  merge myfolder.sum_all(in=in_all) myfolder.sum_mv;
  by seqn day;
  if in_all;
...
*---------------------------------------------------------------------------------------------*;
*Macro %bouts_4of5 defines activity bouts for 4 out of 5 minutes with intensity count >= the*;
*threshold.                                                                                   *;
*                                                                                             *;
*An activity bout starts with a count that is greater than or equal to the threshold.         *;
*A bout is established when 4 minutes out of a 5 minute window have intensity counts greater *;
...
*  - last minute of the day                                                                   *;
*                                                                                             *;
*Macro call %bouts_4of5(bout_flg=);                                                          *;
*bout_flg: variable name for activity bout intensity, _m(moderate), _v(vigorous),             *;
*          _mv(moderate or vigorous)                                                          *;
*---------------------------------------------------------------------------------------------*;
%macro bouts_4of5(bout_flg=);
data myfolder.out5&bout_flg;
  set myfolder.monitors;
  by seqn day paxn;
  /*set up a 10 minute window*/
  array win_paxn(*) win_paxn1-win_paxn5;   /*minute*/
  array win_int(*) win_int1-win_int5;      /*intensity*/
  array win_flg(*) win_flg1-win_flg5;      /*bout flag*/
  ...
  if first.day or stopped or reset then do;
     ...
     sum5=.;        /*the total intensity counts from the 5 minute window*/
     cnt_below=0;   /*counter for number of minutes with intensity below the threshold*/ 
     do i=1 to 5;   /*initialize the 5 minute window*/
        win_paxn(i)=0;
        win_int(i)=0;
        win_flg(i)=0;
     end;
  end;
  retain mv_num reset strt_mv end_mv start mv_cnt found stopped sum5 cnt_below;
  retain win_paxn1-win_paxn5;
  retain win_int1-win_int5;
  retain win_flg1-win_flg5;
  ...
  /*set up a moving window of 5 minutes*/
  if 1<=mv_cnt<=5 and not found then do;
       win_paxn(mv_cnt)=paxn;
       win_int(mv_cnt)=paxinten;
       win_flg(mv_cnt)=&bout_flg;
       if paxinten = . then reset=1; /*if encounter a missing count before reaching the 5 minute count, reset and start again*/
   end;

   /*when reach 5 minutes, count the total number of intensity counts that are >= threshold*/
   if mv_cnt=5 and not reset then sum5=sum(of win_flg1-win_flg5);

   /*if 4 out of 5 minutes with intensity counts >= the threshold, a bout is established*/
   if sum5>=4 then found=1;
   ...
   /*move the 10-minute window down, one minute at a time*/ 
   else if 0<sum5<4 and mv_cnt>5 then do;
     if paxinten=. then reset=1;      /*if the 10th minute has a missing count, reset and start again*/
          ...
          win_int(5)=paxinten;
          win_flg(5)=&bout_flg;
          sum5=sum(of win_flg1-win_flg5);
     end;
   end;
   if sum5 in (0) then reset=1;               /*skip the windows with no valid minutes*/
  ...
  if found then do;
      ...
      if end_mv=0 then do;
         /*the last minute in the 5 minute window is below the threshold*/
         if win_flg(5)=0 then do;
            end_mv=win_paxn(4);
            cnt_below=1;
         end;
         else
...
run;
proc sort data=myfolder.out5&bout_flg;
  by seqn day mv_num;
run;
...
*each day.                                      *;
*-----------------------------------------------*;
proc summary data=myfolder.out5&bout_flg;
  by seqn day;
  var dur_mv;
  output out=myfolder.sum_mv5
         sum=tot_dur_mv5;
run;
...
*the analysis.                                  *;
*-----------------------------------------------*;
data myfolder.out5&bout_flg._sum;
  merge myfolder.sum_all(in=in_all) myfolder.sum_mv5;
  by seqn day;
  if in_all;
  if tot_dur_mv5=. then tot_dur_mv5=0;

  label
  %if &bout_flg=_mv %then %do;
    tot_dur_mv5="Total duration(min) of moderate or vigorous activity bouts (4 out of 5 minutes) in a day"
  %end;
  %if &bout_flg=_m %then %do;
    tot_dur_mv5="Total duration(min) of moderate activity bouts (4 out of 5 minutes) in a day"
  %end;
  %if &bout_flg=_v %then %do;
    tot_dur_mv5="Total duration(min) of vigorous activity bouts (4 out of 5 minutes) in a day"
  %end;
  ;

  keep seqn day tot_dur_mv5;

  rename
  tot_dur_mv5=tot_dur5&bout_flg;
run;
%mend bouts_4of5;

%bouts_4of5(bout_flg=_mv);
%bouts_4of5(bout_flg=_v);
%bouts_4of5(bout_flg=_m);
...
run;
proc sort data=myfolder.out10&bout_flg;
  by seqn day mv_num;
run;
...
*each day.                                      *;
*-----------------------------------------------*;
proc summary data=myfolder.out10&bout_flg;
  by seqn day;
  var dur_mv;
  output out=myfolder.sum_mv10
         sum=tot_dur_mv10;
run;
...
*the analysis.                                  *;
*-----------------------------------------------*;
data myfolder.out10&bout_flg._sum;
  merge myfolder.sum_all(in=in_all) myfolder.sum_mv10;
  by seqn day;
  if in_all;
  if tot_dur_mv10=. then tot_dur_mv10=0;

  label
  %if &bout_flg=_mv %then %do;
    tot_dur_mv10="Total duration(min) of moderate or vigorous activity bouts (8 out of 10 minutes) in a day"
  %end;
  %if &bout_flg=_m %then %do;
    tot_dur_mv10="Total duration(min) of moderate activity bouts (8 out of 10 minutes) in a day"
  %end;
  %if &bout_flg=_v %then %do;
    tot_dur_mv10="Total duration(min) of vigorous activity bouts (8 out of 10 minutes) in a day"
  %end;
  ;

  keep seqn day tot_dur_mv10;

  rename
  tot_dur_mv10=tot_dur10&bout_flg;
run;
%mend bouts_8of10;
...
*-----------------------------------------------------------*;
data myfolder.pam_perday_2005;
  merge myfolder.nw60
        myfolder.out5_mv_sum myfolder.out5_v_sum myfolder.out5_m_sum
		myfolder.out10_mv_sum myfolder.out10_v_sum myfolder.out10_m_sum
        myfolder.out_mv1_sum myfolder.out_v1_sum myfolder.out_m1_sum 
...
proc contents data=myfolder.pam_perday_2005;
run;
